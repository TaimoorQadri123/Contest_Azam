...
if(isset($_GET['pId'])){
    $productId = $_GET['pId'];
    $query = $pdo->prepare("select products.id,products.name as pName ,products.price,products.location,products.description,products.image, categories.name as cName ,categories.id as cId from products inner join categories on products.c_id = categories.id where products.id = :pId");
